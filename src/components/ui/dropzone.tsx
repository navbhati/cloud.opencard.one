// it's a custom dropzone component for file uploads not part of shadcn/ui
"use client";

import { CloudUpload, File, FileSpreadsheet, X } from "lucide-react";
...
type DropzoneProps = {
  ...
  accept?: string;
  file: File | null;
  onFileChange: (file: File | null, meta?: DropzoneChangeMeta) => void;
  helperText?: string;
  maxSizeText?: string;
  disabled?: boolean;
};
...
export function Dropzone({
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 1) {
      onFileChange(files[0], { hasMultipleFiles: true });
      return;
    }
    onFileChange(files?.[0] ?? null, { hasMultipleFiles: false });
  };
  ...
  const resetFile = () => {
    onFileChange(null, { hasMultipleFiles: false });
  };
    ...
    <div className="w-full">
      ...
      >
        <div className="text-center cursor-pointer">
          ...
          <div className="mt-2 flex flex-wrap justify-center text-sm leading-6 text-muted-foreground">
      ...
      {(helperText || maxSizeText) && (
        <p className="mt-2 text-xs leading-5 text-muted-foreground flex items-center justify-between">
      ...
      {file && (
        <Card className="relative mt-4 bg-muted p-3">
          <Button
            type="button"
            variant="ghost"
            size="icon"
          ...
          <div className="flex items-center space-x-2.5">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-sm bg-background shadow-sm ring-1 ring-inset ring-border">
